//
//  AEAssistant_Category.h
//  AEAssistant_Category
//
//  Created by Qian Ye on 16/4/25.
//  Copyright © 2016年 StarDust. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for AEAssistant_Category.
FOUNDATION_EXPORT double AEAssistant_CategoryVersionNumber;

//! Project version string for AEAssistant_Category.
FOUNDATION_EXPORT const unsigned char AEAssistant_CategoryVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <AEAssistant_Category/PublicHeader.h>

//NSDate
#import "NSDate+Convertion.h"
#import "NSDate+CommonDate.h"

//NSString
#import "NSString+DetectEmoji.h"
#import "NSString+MD5Addition.h"
#import "NSString+UrlEncode.h"
#import "NSString+XXTEA.h"
#import "NSString+UrlEscape.h"
#import "NSString+Html.h"
#import "NSString+StringEncoding.h"
#import "NSString+Hash.h"
#import "NSString+SubString.h"
#import "NSString+Convertion.h"
#import "NSString+Bounding.h"

//NSAttributedString
#import "NSAttributedString+Attributes.h"

//UIButton
#import "UIButton+Color.h"

//UIColor
#import "UIColor+GColor.h"

//UIDevice
#import "UIDevice+IdentifierAddition.h"
#import "UIDevice+DeviceInfo.h"

//UIImage
#import "UIImage+GImageExtension.h"

//UINavigationBar
#import "UINavigationBar+BackgroundColor.h"

//UITabbar
#import "UITabBar+Badge.h"

//UIView
#import "UIView+AutoLayout.h"
#import "UIView+ViewHierarchy.h"
#import "UIView+Creation.h"
#import "UIView+Snapshot.h"
#import "UIView+Rendering.h"

